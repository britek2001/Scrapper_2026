...
def load_player_urls_from_config() -> list:
    """Charge player_urls depuis config/players.json (clé 'player_urls'). Lève une erreur si le fichier n'existe pas ou est invalide."""
    config_path = os.path.join(os.path.dirname(__file__), "config", "players.json")
    
    if not os.path.exists(config_path):
        raise FileNotFoundError(f"Fichier de configuration requis non trouvé: {config_path}")
    
    try:
        with open(config_path, 'r', encoding='utf-8') as f:
            cfg = json.load(f)
        
        # support: either {"player_urls": [...]} or a top-level list
        if isinstance(cfg, dict) and 'player_urls' in cfg and isinstance(cfg['player_urls'], list):
            urls = [u for u in cfg['player_urls'] if isinstance(u, str)]
            if not urls:
                raise ValueError("La liste 'player_urls' est vide dans config/players.json")
            return urls
        
        if isinstance(cfg, list):
            urls = [u for u in cfg if isinstance(u, str)]
            if not urls:
                raise ValueError("La liste dans config/players.json est vide")
            return urls
        
        raise ValueError("Format invalide pour config/players.json. Attendu: {'player_urls': [...]} ou [...]")
        
    except json.JSONDecodeError as e:
        raise ValueError(f"Erreur de parsing JSON dans config/players.json: {e}")
    except Exception as e:
        raise RuntimeError(f"Erreur lors du chargement de config/players.json: {e}")

# Chargement des URLs depuis la configuration
PLAYER_URLS = load_player_urls_from_config()
